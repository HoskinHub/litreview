...
color_map = {'PM10': '#332288', 'NO2': '#117733', 'DEHP': '#882255', 'ozone': '#E8CA33', 'PM2.5': '#AA4499', 'multiple': '#88CCEE', 'radon': '#882255'}
...
#ticks = plt.gca().get_yticks()  # Get current y-ticks
#plt.yticks(ticks=ticks)  
# Set y-limits first (before calling plt.yticks())
plt.ylim(-250, 220)
# Create y-ticks at intervals of 250
y_ticks = np.arange(-250, 2201, 250)  # From -250 to 2200 with step of 250
...
for pollutant, marker in marker_map.items():
    # Filter the data for each pollutant
    pollutant_mask = filtration_mask & (df_sorted['benefit_pollutant'] == pollutant)
...
#Zoe adjust ticks and ylims
# Formatting the plot
plt.ylim(-250, 2250)
y_ticks = np.arange(-250, 2201, 250)  # From -250 to 2200 with step of 250
# Set the y-ticks explicitly
plt.yticks(ticks=plt.gca().get_yticks(), labels=[])
plt.xticks(rotation=45, ha='right', fontsize=8)
plt.axhline(0, color='grey', linestyle='--', linewidth=1)
plt.legend(title='Pollutants')
plt.ylabel(' ')
