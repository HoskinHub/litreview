...
hatch_map = {'health': '*', 'performance': '//', 'both': ''}  # Always have a hatch for all benefit types
...
def get_marker_style_and_fill(row):
    ...
    pollutant_marker_map = {'PM10': 'o', 'PM2.5': 's', 'nox': 'D', 'ozone': '^', 'dehp': 'p', 'combination': '*'}
...
for index, row in df[ventilation_mask].iterrows():
        pollutant = row['benefit_pollutant']
        benefit_healthorprod = row['benefit_healthorprod']
        marker, is_filled = get_marker_style_and_fill(row)  # Get marker shape and fill style
        
        # Plot points for this pollutant using the determined marker and fill style
        if is_filled == True:
            # Filled marker (default)
            sns.scatterplot(
                x=[row['benefit_citation']], 
                y=[row['netnew']], 
                marker=marker, 
                label=pollutant if index == df[ventilation_mask].index[0] else "", 
                edgecolor='black',  # Optional: add edge color to make it clearer
                facecolor='black',  # Filled with black (or any color you prefer)
                legend=False
            )
        elif is_filled == False:
            # Hollow marker (edgecolor but no facecolor)
            sns.scatterplot(
                x=[row['benefit_citation']], 
                y=[row['netnew']], 
                marker=marker, 
            ...
            # Half-filled marker using hatch patterns (you can experiment with hatch patterns)
            # Half-filled markers can be simulated using a dashed or hatch pattern
            plt.scatter(
                [row['benefit_citation']], 
                [row['netnew']], 
                marker=marker, 
                edgecolor='black', 
                facecolor='white',  # Fill color as white
                label=pollutant if index == df[ventilation_mask].index[0] else "",
                hatch='/'  # Simulate a half-filled look with a hatch pattern (you can adjust the hatch style)
            )
# Formatting the plot
plt.ylim(-250, 2200)
