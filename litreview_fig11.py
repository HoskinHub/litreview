...
hatch_map = {'health': 'OO', 'performance': '..', 'both': '++'}  # Always have a hatch for all benefit types
...
for index, row in df_sorted[ventilation_mask].iterrows():
    ...
    hatch = hatch_map.get(benefit_healthorprod, 'X')  # If no match in hatch_map, default to 'X'
    
    # Plot the point with the hatch
    plt.scatter(row['benefit_citation'], row['benefit_net'], s=50, marker=marker, alpha=0.5, 
                edgecolors=color, facecolors='none', hatch=hatch)  # 'facecolors' set to 'none' for hatch to show
